...
class Hearing(BaseModel):
    ...
    status: Literal["Cannot hear at all", "Yes, some difficulty", "Yes, a lot of difficulty", "No difficulty"]
    

@router_hearing.post("/hearing")
@cache(expire=60 * 5)
def get_hearing(user_input: Hearing):
    
    con = get_db()
    hearing = con.sql(f"""SELECT District,
    ...
    statuses = list(hearing['status'].unique())

    hearing = hearing.pivot(index='District', values=user_input.age_column, columns='status')
    
    hearing = hearing.div(hearing.sum(axis=1), axis=0) * 100

    hearing = hearing.round(2)

    hearing = hearing.reset_index()

    hearing = hearing.melt(id_vars='District', value_vars=statuses, value_name=user_input.age_column)
    
    hearing = hearing[hearing['status']==user_input.status][['District', user_input.age_column]]

    selected = shp.merge(hearing, on='District').merge(hearing, left_on='Region', right_on='District').drop(['District_y'], axis=1)
    ...
    selected['National_percentage'] = hearing[hearing['District']=='Ghana'][user_input.age_column].values[0]
