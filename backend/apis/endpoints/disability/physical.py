...
@router_physical.post("/physical")
@cache(expire=60 * 5)
def get_physical(user_input: Physical):
    
    con = get_db()
    physical = con.sql(f"""SELECT District,
    ...
    statuses = list(physical['status'].unique())

    physical = physical.pivot(index='District', values=user_input.age_column, columns='status')
    
    physical = physical.div(physical.sum(axis=1), axis=0) * 100

    physical = physical.round(2)

    physical = physical.reset_index()

    physical = physical.melt(id_vars='District', value_vars=statuses, value_name=user_input.age_column)
    
    physical = physical[physical['status']==user_input.status][['District', user_input.age_column]]

    selected = shp.merge(physical, on='District').merge(physical, left_on='Region', right_on='District').drop(['District_y'], axis=1)
    ...
    selected['National_percentage'] = physical[physical['District']=='Ghana'][user_input.age_column].values[0]
