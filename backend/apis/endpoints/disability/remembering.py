...
router_remembering = APIRouter()

class Remembering(BaseModel):
    ...
    status: Literal[ "Cannot remember or concentrate at all", "Yes, some difficulty", "Yes, a lot of difficulty", "No difficulty"]
    

@router_remembering.post("/remembering")
@cache(expire=60 * 5)
def get_remembering(user_input: Remembering):

    con = get_db()
    remembering = con.sql(f"""SELECT District,
    ...
    statuses = list(remembering['status'].unique())

    remembering = remembering.pivot(index='District', values=user_input.age_column, columns='status')
    
    remembering = remembering.div(remembering.sum(axis=1), axis=0) * 100

    remembering = remembering.round(2)

    remembering = remembering.reset_index()

    remembering = remembering.melt(id_vars='District', value_vars=statuses, value_name=user_input.age_column)
    
    remembering = remembering[remembering['status']==user_input.status][['District', user_input.age_column]]

    selected = shp.merge(remembering, on='District').merge(remembering, left_on='Region', right_on='District').drop(['District_y'], axis=1)
    ...
    selected['National_percentage'] = remembering[remembering['District']=='Ghana'][user_input.age_column].values[0]
