...
router_seeing = APIRouter()

class Seeing(BaseModel):
    ...
    status: Literal["No difficulty","Cannot see at all", "Yes, some difficulty", "Yes, a lot of difficulty"]
    

@router_seeing.post("/seeing")
@cache(expire=60 * 5)
def get_seeing(user_input: Seeing):
    
    con = get_db()
    seeing = con.sql(f"""SELECT District,
    ...
    statuses = list(seeing['status'].unique())

    seeing = seeing.pivot(index='District', values=user_input.age_column, columns='status')
    
    seeing = seeing.div(seeing.sum(axis=1), axis=0) * 100

    seeing = seeing.round(2)

    seeing = seeing.reset_index()

    seeing = seeing.melt(id_vars='District', value_vars=statuses, value_name=user_input.age_column)
    
    seeing = seeing[seeing['status']==user_input.status][['District', user_input.age_column]]

    selected = shp.merge(seeing, on='District').merge(seeing, left_on='Region', right_on='District').drop(['District_y'], axis=1)
    ...
    selected['National_percentage'] = seeing[seeing['District']=='Ghana'][user_input.age_column].values[0]
