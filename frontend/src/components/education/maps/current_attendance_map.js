...
import CurrentAttendanceFilters from "../filters/filters_current_attendance_map";
...
function  CurrentAttendanceMap() {
  ...
  const updateMap = async (newFilters) => {
    const res = await fetch("http://127.0.0.1:8000/current_attendance", {
      method: "POST",
        ...
        function getQuantiles(values, numQuantiles) {
          ...
          for (let i = 1; i < numQuantiles; i++) {
            ...
            if (values[base + 1] !== undefined) {
                let quant_val = values[base] + rest * (values[base + 1] - values[base])
              quantiles.push(
                quant_val.toFixed(2)
              );
            } else {
    ...
    geoJsonLayerRef.current = L.geoJSON(data, {
          ...
          onEachFeature: (feature, layer) => {
            ...
            layer.bindPopup(
              ...
              Percentage: ${feature.properties[filters.age_column]}%`
            );
      ...
      <div className="container-fluid">
        <div className="row">
          ...
          >
            ...
            <CurrentAttendanceFilters filters={filters} setFilters={setFilters} />
          </div>
...
export default CurrentAttendanceMap;
