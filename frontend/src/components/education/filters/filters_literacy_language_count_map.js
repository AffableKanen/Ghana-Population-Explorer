...
export default function LiteracyLanguageCountFilters({ filters, setFilters }) {
  return (
    <div className="filters">
      ...
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "120px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
        <label className="fw-bold text-decoration-underline" style={{ color: "white" }}>LANGUAGE COUNT</label>
        {['Literate in one language','Literate in two languages','Literate in three languages','Literate in more than three languages'].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
              name="language_count"
              id={`language-count-${opt}`}
              value={opt}
              checked={filters.language_count === opt}
              onChange={(e) =>
                setFilters({ ...filters, language_count: e.target.value })
              }
            />
            <label className="form-check-label" htmlFor={`language-count-${opt}`} style={{ color: "white" }}>
              {opt}
            </label>
          </div>
        ))}
