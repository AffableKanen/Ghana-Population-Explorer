...
export default function AttendanceStatusFilters({ filters, setFilters }) {
  return (
    <div className="filters">
        <h4 className="text-center" style={{ color: "#fff", backgroundColor: "#702A2F", padding: "10px", borderRadius: "5px" }}> Filter by: </h4>
      {/* Age Column */}
      <div style={{ border: "2px solid #800020", padding: "8px", marginBottom: "6px", maxHeight: "250px", overflowY: "auto", backgroundColor: "#b33655ff" }}>
      ...
      {/* Locality */}
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "140px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
      ...
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "120px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
        <label className="fw-bold text-decoration-underline" style={{ color: "white" }}>ATTENDANCE STATUS</label>
        {['Attended in the past', 'Attending now', 'Never attended'].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
              name="attendance_status"
              id={`attendance-status-${opt}`}
              value={opt}
              checked={filters.attendance_status === opt}
              onChange={(e) =>
                setFilters({ ...filters, attendance_status: e.target.value })
              }
            />
            <label className="form-check-label" htmlFor={`attendance-status-${opt}`} style={{ color: "white" }}>
              {opt}
            </label>
          </div>
        ))}
