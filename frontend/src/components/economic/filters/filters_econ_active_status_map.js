...
export default function EconActiveStatusFilters({ filters, setFilters }) {
  return (
    ...
    {/* Status */}
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "120px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
        <label className="fw-bold text-decoration-underline" style={{ color: "white" }}>ECONOMIC STATUS</label>
        {["Employed","Worked","Did not work but had a job","Unemployed","Available but not seeking work","First time job seeker", 
        "Worked previously & seeking work", "Outside Labour Force"].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
              name="economic_status"
              id={`economic_status-${opt}`}
              value={opt}
              checked={filters.economic_status === opt}
              onChange={(e) =>
                setFilters({ ...filters, economic_status: e.target.value })
              }
            />
            <label className="form-check-label" htmlFor={`economic_status-${opt}`} style={{ color: "white" }}>
              {opt}
            </label>
          </div>
        ))}
