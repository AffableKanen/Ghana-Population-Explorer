...
export default function EconActiveOccupationFilters({ filters, setFilters }) {
  return (
    <div className="filters">
      ...
      <div style={{ border: "2px solid #800020", padding: "8px", marginBottom: "6px", maxHeight: "140px", overflowY: "auto", backgroundColor: "#b33655ff" }}>
        ...
        {["Total","Never attended","Nursery","Kindergarten","Primary","JSS/JHS","Middle","SSS/SHS","Secondary",
           ...
           "Tertiary - PhD", "Other (specify)"].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
    ...
    {/* Occupation */}
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "150px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
        <label className="fw-bold text-decoration-underline" style={{ color: "white" }}>OCCUPATION</label>
        {["Managers","Professionals","Technicians and associate professionals","Clerical support workers","Service and sales workers",
          ...
          "Elementary occupation workers", "Other occupations"].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
              ...
              id={`occupations-${opt}`}
              value={opt}
              checked={filters.occupations === opt}
              onChange={(e) =>
                setFilters({ ...filters, occupations: e.target.value })
              }
            />
            <label className="form-check-label" htmlFor={`occupations-${opt}`} style={{ color: "white" }}>
              {opt}
            </label>
          </div>
        ))}
