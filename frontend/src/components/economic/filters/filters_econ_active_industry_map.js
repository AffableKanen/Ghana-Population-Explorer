...
export default function EconActiveIndustryFilters({ filters, setFilters }) {
  return (
    ...
    {/* Industries */}
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "120px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
        ...
        {["Agriculture, forestry and fishing","Manufacturing","Transportation and storage","Wholesale and retail trade; repair of motor vehicles and motorcycles",
          ...
          "Activities of extraterritorial organizations and bodies"].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
              ...
              id={`industries-${opt}`}
              value={opt}
              checked={filters.industries === opt}
              onChange={(e) =>
                setFilters({ ...filters, industries: e.target.value })
              }
            />
            <label className="form-check-label" htmlFor={`industries-${opt}`} style={{ color: "white" }}>
              {opt}
            </label>
          </div>
        ))}
