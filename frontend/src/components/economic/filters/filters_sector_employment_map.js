...
export default function SectorEmploymentFilters({ filters, setFilters }) {
  return (
    ...
    {/* sector */}
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "150px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
        <label className="fw-bold text-decoration-underline" style={{ color: "white" }}>SECTOR</label>
        {["Public (Government)","Semi-Public/Parastatal","Private Formal","Private Informal",
            "Local NGO/CSO","International NGO/CSO","Religious Organization (local)","Religious Organization (international)",
            "International Organization"].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
              ...
              id={`sectors-${opt}`}
              value={opt}
              checked={filters.sectors === opt}
              onChange={(e) =>
                setFilters({ ...filters, sectors: e.target.value })
              }
            />
            <label className="form-check-label" htmlFor={`sectors-${opt}`} style={{ color: "white" }}>
              {opt}
            </label>
          </div>
        ))}
