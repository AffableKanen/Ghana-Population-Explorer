...
export default function EmploymentClassificationFilters({ filters, setFilters }) {
  return (
    ...
    {/* status */}
      <div style={{ border: "2px solid #c42f54ff", padding: "8px", marginBottom: "6px", maxHeight: "150px", overflowY: "auto", backgroundColor: "#b33655ff"}}>
        <label className="fw-bold text-decoration-underline" style={{ color: "white" }}>CLASSIFICATION</label>
        {['Employee', 'Self-employed without employees', 'Self-employed with employees', 'Casual worker', 'Contributing family worker', 
          'Paid apprentice', 'Unpaid apprentice', 'Domestic worker', 'Other'].map((opt) => (
          <div className="form-check" key={opt}>
            <input
              className="form-check-input"
              type="radio"
            ...
            />
            <label className="form-check-label" htmlFor={`status-${opt}`} style={{ color: "white" }}>
              {opt}
            </label>
          </div>
        ))}
